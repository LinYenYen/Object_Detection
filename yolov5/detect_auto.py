...
def run(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
    ...
    save_dir = Path(project) / name  # remain original run
    ...
    #                                                                     Auto Mode                                                                      #
    ######################################################################################################################################################
    print(f'\n{"*"*10}Start Auto Detect{"*"*10}')

    start_time = datetime.now()
    img_source = Path(source)
    while True:
        # 檢查資料夾內是否有新增檔案
        images = [os.fspath(img) for img in img_source.iterdir() if datetime.fromtimestamp(img.stat().st_ctime) > start_time]
        start_time = datetime.now()     # 檢查完立刻刷新時間
        if images:
            for source in images:       # 這裡的 source 為單張照片的路徑
        ...
        time.sleep(1)       # 每檢查一次休息 1 秒鐘，避免程式過度運轉
